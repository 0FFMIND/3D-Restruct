                    ...
                    if id == 11:
                        ...
                        if opcode == 2:
                            ...
                            if sequenceNum == 1:
                                ...
                                value.append(state_mean_value)
                                valuesKalman.append(str(time.time() - begin) + ',' + str(state_mean_value))
                            else:
                                state_mean_value = Kalman1D(cy, 0.013, value[sequenceNum-2], 0.01)
                                value.append(state_mean_value)
                                valuesKalman.append(str(time.time() - begin) + ',' + str(state_mean_value))
                        if opcode == 3:
                            window_length = 7
                        ...
                        if opcode == 4:
                            ...
                            previousCy2 = cy
                        if opcode == 5:
                            values5.append(str(now - begin) + ',' + str(cy))
                            if sequenceNum == 1:
                                state_mean_value = Kalman1D(cy, 0.013, cy, 0.01)
                                value.append(state_mean_value)
                            else:
                            ...
                            polyorder = 3
                            if sequenceNum >= window_length:
                                y_smoothed = savgol_filter(value[-window_length:], window_length, polyorder)
                                valuesMix2.append(str(time.time() - begin) + ',' + str(y_smoothed[-1]))
                            else:
                                valuesMix2.append(str(time.time() - begin) + ',' + str(state_mean_value))
            else:
                continue
        else:
            break
...
numOrders1, numOrders2, numOrders3, numOrders4, numOrders5 = [], [], [], [], []
numCys1, numCys2, numCys3, numCys4, numCys5 = [], [], [], [], []
numCy1, numCy2, numCy3, numCy4, numCy5 = [], [], [], [], []
...
Flatten(valuesMix2, numOrders5, numCys5)
Flatten(values5, num5, numCy5)

#plt.plot(num1, numCy1, label='Original', color='blue')
...
#plt.plot(numOrders4, numCys4, label='Mixed Filtered', color='black', alpha=1)
#plt.plot(num5, numCy5, label='Original', color='blue')
#plt.plot(numOrders5, numCys5, label='Mixed2 Filtered', color='yellow', alpha=1)
